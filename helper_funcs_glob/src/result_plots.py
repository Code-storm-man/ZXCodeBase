...
def result_plots(plot_opts: dict,
                 width_veh_opt_r: float,
                 width_veh_opt_l: float,
    ...
    if plot_opts["raceline"]:
        ...
        veh_bound1_virt = trajectory[:, 1:3] + normvec_normalized_opt * width_veh_opt_r / 2
        veh_bound2_virt = trajectory[:, 1:3] - normvec_normalized_opt * width_veh_opt_l / 2
        ...
        # # 将数据组合成一个列表
        # data = list(zip(veh_bound1_real[:, 0], veh_bound1_real[:, 1], veh_bound2_real[:, 0], veh_bound2_real[:, 1]))
        #
        # # 指定 CSV 文件路径
        # csv_file_path = "vehicle_bounds.csv"
        #
        # # 写入 CSV 文件
        # with open(csv_file_path, mode='w', newline='') as file:
        #     writer = csv.writer(file)
        #     # 写入表头
        #     writer.writerow(["veh_bound1_real_x", "veh_bound1_real_y", "veh_bound2_real_x", "veh_bound2_real_y"])
        #     # 写入数据
