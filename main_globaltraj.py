...
# file_paths["track_name"] = "BaseLine_suzuka_forjiang"
# file_paths["track_name"] = "Smoothed_Modified_BaseLine_suzuka_forjiang_v11"
file_paths["track_name"] = "yas_tum_v3"
...
mintime_opts = {"tpadata": None,
                ...
                "recalc_vel_profile_by_tph": True}  # 是否通过那个重新计算速度
...
lap_time_mat_opts = {"use_lap_time_mat": False,             # calculate a lap time matrix (diff. top speeds and scales)
                     ...
                     "top_speed_range": [100.0, 350.0],     # range of top speeds to be simulated [in km/h]
...
# 这里我没有进去，是为了进行对路径进行二次优化，现在的路径没必要
# if opt_type == 'mintime' and mintime_opts["reopt_mintime_solution"]:
#
...
# ----------------------------------------------------------------------------------------------------------------------
# 这里输出的raceline_interp是插值之后的raceline路径点
# t_vals_opt_interp是三次样条的那个输入量t
...
# calc_head_curv_an 使用三次样条系数（coeffs_x 和 coeffs_y）计算航向角和曲率。每个插值点需要对应的样条段系数来计算其一阶和二阶导数。
# ind_spls[i] 指定第 i 个插值点所属的样条段索引 j，从而从 coeffs_x[j, :] 和 coeffs_y[j, :] 中提取正确的系数：
# 例如，若 ind_spls[i] = 2，则使用 coeffs_x[2, :] 和 coeffs_y[2, :] 来计算第 i 个点的导数。
...
vx_profile_opt_cl = np.append(vx_profile_opt, vx_profile_opt[0])  # 得到的速度也要做前后的闭合  这里已经是插值后的路径计算的
ax_profile_opt = tph.calc_ax_profile.calc_ax_profile(vx_profile=vx_profile_opt_cl,
                                                     el_lengths=el_lengths_opt_interp,
                                                     eq_length_output=False)

# calculate laptime
# alc_t_profile 函数的用途是根据给定的速度剖面（vx_profile）和路径点间的元素长度（el_lengths），计算赛车线每个点的时间剖面（t_profile）
...
if "traj_ltpl_export" in file_paths.keys():
    helper_funcs_glob.src.export_traj_ltpl.export_traj_ltpl(file_paths=file_paths,
                                                            spline_lengths_opt=spline_lengths_opt,#(1573)
                                                            trajectory_opt=trajectory_opt,#(2625)
                                                            reftrack=reftrack_interp,#(1573)
                                                            normvec_normalized=normvec_normalized_interp,#(1573)
                                                            alpha_opt=alpha_opt)
...
helper_funcs_glob.src.result_plots.result_plots(plot_opts=plot_opts,
                                                width_veh_opt_r=pars["optim_opts"]["width_opt_r"],
                                                width_veh_opt_l=pars["optim_opts"]["width_opt_l"],
