...
def opt_mintime(reftrack: np.ndarray,
    ...
    f_xdrag = -(-1.04765 * v ** 2 + 47.5 * v - 1609.73)
    ...
    ax = (f_x_rl + f_x_rr + (f_x_fl + f_x_fr) * ca.cos(delta) - (f_y_fl + f_y_fr) * ca.sin(delta)
          - (-(-1.04765 * v ** 2 + 47.5 * v - 1609.73))) / mass
    ...
    Xk = ca.MX.sym('X0', nx)
    w.append(Xk)
    n_min = (-w_tr_right_interp(0) + pars["optim_opts"]["width_opt_r"]) / n_s
    n_max = (w_tr_left_interp(0) - pars["optim_opts"]["width_opt_l"]) / n_s
    if pars["pwr_params_mintime"]["pwr_behavior"]:
    ...
    for k in range(N):
        ...
        w.append(Xk)
        n_min = (-w_tr_right_interp(k + 1) + pars["optim_opts"]["width_opt_r"]) / n_s
        n_max = (w_tr_left_interp(k + 1) - pars["optim_opts"]["width_opt_l"]) / n_s
        if pars["pwr_params_mintime"]["pwr_behavior"]:
            lbw.append([v_min, beta_min, omega_z_min, n_min, xi_min,
                        machine.temp_min, batt.temp_min, inverter.temp_min,
                        radiators.temp_cool_mi_min, radiators.temp_cool_b_min,
